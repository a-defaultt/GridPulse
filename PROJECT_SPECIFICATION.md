...
*   **Newsletter Sheet Link (V5.9):** `GOOGLE_SHEET_URL` is derived from `GOOGLE_SHEET_ID` and injected into every daily/weekly/monthly template render — a styled button above the footer plus a plain-text fallback line — so recipients can jump straight to the live, continuously updated IOC feed instead of a point-in-time attachment.
*   **Downstream Consumer:** The same sheet is read (read-only service account) by **Sentinel**, which cross-references live SIEM telemetry against it and escalates matching alerts automatically — see `Sentinel/Project_Sentinel_Spec_v3.md` §7–8.
